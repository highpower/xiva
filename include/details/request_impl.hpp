...
request_impl::parse_query_string(range<Iter> const &query) {
	...
	while (!r.empty()) {
		split_first_of_once(r, http_constants::param_delimiter, part, r);
...
request_impl::parse_request_line_impl(range<Iter> const &line) {
	...
	if (!is_ci_equal(http_constants::get, method)) {
...
request_impl::parse_headers_line_impl(range<Iter> const &line) {
	...
	if (is_ci_equal(http_constants::range_name, head)) {
		throw http_error(http_error::range_not_satisfiable);
	}
	else if (is_ci_equal(http_constants::cookie, head)) {
		parse_cookies(tail);
	}
	headers_.insert(std::make_pair(std::string(head.begin(), head.end()),
...
request_impl::is_required_protocol_version(range<Iter> const &r) const {
	return is_ci_equal(http_constants::protocol_version, r);
}

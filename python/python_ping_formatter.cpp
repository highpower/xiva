...
python_ping_formatter::message() const {
	...
	try {
		...
		return py::extract<std::string>(py::call_method<py::str>(impl_->ptr(), "message"));
	}
	catch (std::exception const &) {
		throw;
	}
	catch (...) {
		throw std::runtime_error("caught unknown exception in python_formatter::message");
	}
}
...
python_ping_formatter::message_for_single() const {
	...
	try {
		interpreter_lock lock;
		return py::extract<std::string>(py::call_method<py::str>(impl_->ptr(), "message_for_single"));
	}
	catch (std::exception const &) {
		throw;
	}
	catch (...) {
		throw std::runtime_error("caught unknown exception in python_formatter::message_for_single");
	}
}
